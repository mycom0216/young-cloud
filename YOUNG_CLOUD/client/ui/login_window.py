...
class SignUpDialog(QDialog, Ui_Form):
    ...
    def __init__(self, parent=None):
        ...
        self.pushButton_7.clicked.connect(self.handle_company_api)    # 회사명 옆 검색 버튼 필요시 처리
        
    
    def handle_company_api(self):
        # TODO: 추후 회사 검색 API 연동 예정
        from PySide6.QtWidgets import QMessageBox
        QMessageBox.information(self, "안내", "회사 검색 API는 추후 연동될 예정입니다.")
    ...
    def check_password_valid(self):
        ...
        # 영문, 숫자, 특수문자가 모두 포함된 8~20자 정규식
        pattern = r"^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%^&*()_+~`-{}\[\]:óloga;\"'<>,.?/\\|]).{8,20}$"
    ...
    def handle_signup(self):
        ...
        if response.get("status") == "success":
            QMessageBox.information(self, "환영합니다", "회원가입이 완료되었습니다! 로그인해주세요.")
            self.accept() # 다이얼로그 닫기
        else:
            QMessageBox.critical(self, "가입 실패", response.get("message", "회원가입 중 오류가 발생했습니다."))
...
class LoginWindow(QWidget):
    ...
    def init_ui(self):
        ...
        image_path = os.path.join(root_dir, "client", "ui", "source", "image", "YOUNG CLOUD.png")
